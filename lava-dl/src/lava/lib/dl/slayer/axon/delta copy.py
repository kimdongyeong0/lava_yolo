...
class _DeltaUnit(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            ...
            # spike rate를 surrogate gradient를 계산하는데 lambda함수의 factor로 사용하기 위해 전달
            torch.autograd.Variable(
                torch.tensor(
                    output,
                    device=input.device,
        ...
        print("spike in foward ", event_rate(output))
    ...
    def backward(ctx, grad_output, *_):
        ...
        delta_input, threshold, tau_grad, scale_grad, output = ctx.saved_tensors # 임시로 output을 추가해서 spike rate를 lambda의 factor로 사용

        print("spike in backward", event_rate(output))
