...
# ---------------------------------------------------------------------------
# Residual Block Definition using Sigma-Delta Convolutional Blocks
# ---------------------------------------------------------------------------
class ResidualBlock(torch.nn.Module):
    """
    Residual block composed of two sigma-delta convolutional layers.
    Uses a 1x1 sigma-delta convolution as shortcut if input and output channels differ.
    """
    def __init__(self, neuron_kwargs, in_channels: int, out_channels: int, block_kwargs: dict):
        super(ResidualBlock, self).__init__()
        self.conv1 = slayer.block.sigma_delta.Conv(
            neuron_kwargs, in_channels, out_channels, kernel_size=3,
            padding=1, stride=1, weight_scale=1, **block_kwargs
        )
        self.conv2 = slayer.block.sigma_delta.Conv(
            neuron_kwargs, out_channels, out_channels, kernel_size=3,
            padding=1, stride=1, weight_scale=1, **block_kwargs
        )
        if in_channels != out_channels:
            self.shortcut = slayer.block.sigma_delta.Conv(
                neuron_kwargs, in_channels, out_channels, kernel_size=1,
                padding=0, stride=1, weight_scale=1, **block_kwargs
            )
        else:
...
class Network(YOLOBase):
    ...
    def __init__(self,
        ...
        # Define backbone network with residual blocks for better feature extraction
        self.blocks = torch.nn.ModuleList([
            ...
            # Fourth residual block - 256 channels
            ResidualBlock(neuron_kwargs, 256, 256, block_8_kwargs),
            ...
            slayer.block.sigma_delta.Conv(neuron_kwargs, 256, 512, 3, padding=1, stride=1, weight_scale=3, **block_8_kwargs),
            slayer.block.sigma_delta.Conv(neuron_kwargs, 512, 512, 3, padding=1, stride=1, weight_scale=3, **block_8_kwargs),
            slayer.block.sigma_delta.Conv(neuron_kwargs,  512, 1024, 3, padding=1, stride=1, weight_scale=3, **block_8_kwargs),
            slayer.block.sigma_delta.Conv(neuron_kwargs, 1024,  256, 1, padding=0, stride=1, weight_scale=3, **block_8_kwargs),
        ])
    ...
    def export_hdf5(self, filename: str) -> None:
        """Export the YOLO-KP network as an HDF5 description.
        ...
        """
        # Create HDF5 file and a group for layers
        ...
        # Create lists of blocks to export.
        # Here we assume self.blocks contains the backbone layers,
        # and additional modules like upsample_conv and residual_fusion are wrapped in lists.
        block_lists = [
            self.blocks,
        ]

        # Iterate over the blocks and export those that are not ResidualBlock
        for blocks in block_lists:
            for b in blocks:
                if isinstance(b, ResidualBlock):
                    print(f"Skipping export of ResidualBlock at index {offset}")
                    continue
                # Check if the block has the export_hdf5 method before calling it
                if hasattr(b, 'export_hdf5'):
                    b.export_hdf5(layer.create_group(f'{offset}'))
                offset += 1
        
        if hasattr(self.heads[i], 'export_hdf5'):
            self.heads[0].export_hdf5(layer.create_group(f'{offset}'))
            offset += 1
        
        # Export the head using its dedicated export method
