...
class Network(YOLOBase):
    ...
    def __init__(self,
        ...
        self.head1_backend = torch.nn.ModuleList([
            slayer.block.sigma_delta.Conv(neuron_kwargs,  256,  256, 3, padding=1, stride=1, **block_kwargs),
    ...
    def export_hdf5_head(self, handle: h5py.Dataset, head_index: int) -> None:
        """
        Export a YOLO head to HDF5 format.
        ...
        handle : h5py.Dataset
            HDF5 dataset handle.
        head_index : int
            Index of the head to export (0 for head1, 1 for head2).
        """
        def weight(s):
            return s.pre_hook_fx(s.weight, descale=True).reshape(s.weight.shape).cpu().data.numpy()
        
        handle.create_dataset('type', (1,), 'S10', ['conv'.encode('ascii', 'ignore')])
        
        # Select the appropriate head blocks based on the head_index
        if head_index == 0:
            synapse = self.head1_blocks[1]  # The Conv synapse is at index 1
            neuron = self.head1_blocks[2]   # The Sigma dendrite is at index 2
        else:  # head_index == 1
            synapse = self.head2_blocks[1]  # The Conv synapse is at index 1
            neuron = self.head2_blocks[2]   # The Sigma dendrite is at index 2
    ...
    def export_hdf5(self, filename: str) -> None:
        """
        Export the entire network to HDF5 format for Loihi deployment.
        ...
        # Export input blocks
        for block in self.input_blocks:
            block.export_hdf5(layer.create_group(f'{offset}'))
            offset += 1
        
        # Export backend blocks (skip ResidualBlock)
        for block in self.backend_blocks:
            block.export_hdf5(layer.create_group(f'{offset}'))
            offset += 1
        
        # Export head1 backend blocks
        for block in self.head1_backend:
            block.export_hdf5(layer.create_group(f'{offset}'))
            offset += 1
        
        # Export head1 first Conv block (the Conv block in head1_blocks)
        self.head1_blocks[0].export_hdf5(layer.create_group(f'{offset}'))
        offset += 1
        
        # Export head2 backend blocks
        for block in self.head2_backend:
            block.export_hdf5(layer.create_group(f'{offset}'))
            offset += 1
        
        # Export head2 first Conv block (the Conv block in head2_blocks)
        self.head2_blocks[0].export_hdf5(layer.create_group(f'{offset}'))
        offset += 1
        
        # Export the two heads separately
        self.export_hdf5_head(layer.create_group(f'{offset}'), head_index=0)  # Export head1
        offset += 1
        self.export_hdf5_head(layer.create_group(f'{offset}'), head_index=1)  # Export head2
